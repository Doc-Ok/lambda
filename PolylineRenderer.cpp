...
void PolylineRenderer::initContext(GLContextData& contextData) const
	...
	GLhandleARB vertexShader=glCompileVertexShaderFromFile(LAMBDA_CONFIG_SHADERDIR "/PolylineRenderer.vs");
	...
	if(dataItem->haveCoreGeometryShaders)
		...
		geometryShader=glCompileARBGeometryShader4FromFile(LAMBDA_CONFIG_SHADERDIR "/PolylineRendererCore.gs");
		}
	else
		{
		/* Create an ARB geometry shader: */
		geometryShader=glCompileARBGeometryShader4FromFile(LAMBDA_CONFIG_SHADERDIR "/PolylineRendererARB.gs");
	...
	GLhandleARB fragmentShader=glCompileFragmentShaderFromFile(LAMBDA_CONFIG_SHADERDIR "/PolylineRenderer.fs");
...
GLObject::DataItem* PolylineRenderer::activate(Scalar newPixelSize,GLContextData& contextData) const
	{
	/* Retrieve the context data item: */
	DataItem* dataItem=contextData.retrieveDataItem<DataItem>(this);
	
	/* Store the current pixel size: */
	dataItem->pixelSize=newPixelSize;
	...
	glUniform1fARB(dataItem->uniforms[1],GLfloat(dataItem->pixelSize));
...
void PolylineRenderer::draw(const Polyline& polyline,const Color& color,Scalar lineWidth,GLObject::DataItem* dataItem) const
	{
	/* Retrieve the data item: */
	DataItem* myDataItem=static_cast<DataItem*>(dataItem);
	...
	glVertex2fv(polyline.front().coords);
	for(Polyline::const_iterator pIt=polyline.begin();pIt!=polyline.end();++pIt)
		glVertex2fv(pIt->coords);
	if(polyline.size()==1U)
		glVertex2fv(polyline.back().coords);
	glVertex2fv(polyline.back().coords);
	glEnd();
	}

void PolylineRenderer::draw(const void* cacheId,unsigned int version,const Polyline& polyline,const Color& color,Scalar lineWidth,GLObject::DataItem* dataItem) const
	{
	/* Retrieve the data item: */
	DataItem* myDataItem=static_cast<DataItem*>(dataItem);
	...
	if(myDataItem->currentBufferId!=cmIt->getDest().memoryBlock->bufferId)
		...
		glVertexPointer(2,GL_FLOAT,sizeof(DataItem::Vertex),static_cast<const GLfloat*>(0)+0);
		}
